...
# /genetics/PAREG/perrotn/scripts/IVs_BM_MVMR_PURE.sh

# ADAPT FOR USING THE BM-BM CIS SNPS LIST

########################################
########## Multivariate MR IV ##########
########################################

# UPDATE OF THE MVMR THAT CAN INCLUDE BM AS SECONDARY EXPOSURES
...
count=0
declare -A COL_NUM_SS
for name in $(head -n 1 $SAMPLE_FILE) ; do
let count=count+1
COL_NUM_SS[$name]=$count
done

EXPOSURE_MODEL="UKB_CARDIoGRAMplusC4D_MI_Hartiala_2021"
EXPOSURE_MODEL="EUROPEAN_CMET_F7_F7"
for EXPOSURE_MODEL in $(awk 'BEGIN{FS=OFS="\t"}NR>1{print $1}' $LIST_EXPOSURES)
do
  echo $EXPOSURE_MODEL


    if [ $(awk 'BEGIN{FS=OFS="\t"}NR>1 && $1=="'$EXPOSURE_MODEL'"{print $2}' $LIST_EXPOSURES) != "NA" ] && [ $(awk 'BEGIN{FS=OFS="\t"}NR>1 && $1=="'$EXPOSURE_MODEL'"{print $2}' $LIST_EXPOSURES) != "" ]
    then 
        ...
        if [ $POPULATION_EXP == "METAL_LATIN_EUROPEAN_PERSIAN" ] || [ $POPULATION_EXP == "METAL_ALL_ETHNICITIES" ] ; then
            if [ $POPULATION_EXP == "METAL_LATIN_EUROPEAN_PERSIAN" ] ; then
                SAMPLE_SIZE_EXPOSURE_MODEL=$(grep -w $PANEL_EXP $SAMPLE_FILE | awk 'BEGIN{FS=OFS="\t"} {print $('${COL_NUM_SS["LATIN"]}'), $('${COL_NUM_SS["EUROPEAN"]}'), $('${COL_NUM_SS["PERSIAN"]}')}' | awk '{sum=0; for (i=1; i<=NF; i++) { sum+= $i } print sum}')
                LIST_POP=$(echo "EUROPEAN" "LATIN" "PERSIAN")
            
            else # $POPULATION_EXP == "METAL_ALL_ETHNICITIES"
                SAMPLE_SIZE_EXPOSURE_MODEL=$(grep -w $PANEL_EXP $SAMPLE_FILE | awk 'BEGIN{FS=OFS="\t"} {print $('${COL_NUM_SS["LATIN"]}'), $('${COL_NUM_SS["EUROPEAN"]}'), $('${COL_NUM_SS["PERSIAN"]}'), $('${COL_NUM_SS["AFRICAN"]}'), $('${COL_NUM_SS["SOUTH_ASIAN"]}'), $('${COL_NUM_SS["EAST_ASIAN"]}'), $('${COL_NUM_SS["ARAB"]}')}' | awk '{sum=0; for (i=1; i<=NF; i++) { sum+= $i } print sum}')
                LIST_POP=$(echo "EUROPEAN" "ARAB" "EAST_ASIAN" "LATIN" "PERSIAN" "SOUTH_ASIAN" "AFRICAN")
            fi # END IFELSE $POPULATION_EXP == "METAL_LATIN_EUROPEAN_PERSIAN" 
        else
            SAMPLE_SIZE_EXPOSURE_MODEL=$(grep -w $PANEL_EXP $SAMPLE_FILE | awk 'BEGIN{FS=OFS="\t"} {print $('${COL_NUM_SS[$POPULATION_EXP]}')}')
            if [ $POPULATION_EXP == "EUROPEAN" ]; then
                LIST_POP=$(echo "EUROPEAN")
            else 
                LIST_POP=$(echo "EUROPEAN" $POPULATION_EXP)
            fi
        fi
...
do
  ...
  do
    ...
    awk 'BEGIN{FS=OFS="\t"}NR==1{print}' $LIST_EXPOSURES > ${MODEL}.txt
    awk 'BEGIN{FS=OFS="\t"}NR>1 && $1 == "'$EXPOSURE'"{print}' $LIST_EXPOSURES >> ${MODEL}.txt
    ...
    do
      ...
      do
        ...
        do
              ...
              RUN_PARALLEL 0.95 0.95 ${N_CPU} 8 "$script_log" "$SCRIPT" $SUFFIX $PANEL $ASSAY $GENE ${ROOT_DIR}PURE/MVMR_BM_consortia/${MODEL}.txt $PVALUE_THRESHOLD $CIS_WINDOW $LD_THRESHOLD_PRUNING $OUTCOME $ROOT_DIR $GRS_FORMATING_FILE $MODEL $POPULATION
...
rm -f ${ROOT_DIR}PURE/MVMR_BM_consortia/$ASSAY"_"*
...
LIST_OUTCOMES
SCRIPT="/genetics/PAREG/perrotn/scripts/IVs_BM_MVMR_PURE.sh"
chmod u+x $SCRIPT


LIST_EXPOSURES=/storage/genetics_work2/perrotn/PURE/BM_BM_MR_V3/LIST_EXPOSURES_BM_BM.txt
awk 'BEGIN{FS=OFS="\t"}{print}' "$LIST_EXPOSURES" | tr -d '\r' > "${LIST_EXPOSURES}"temp
mv "${LIST_EXPOSURES}"temp "$LIST_EXPOSURES"


LIST_OUTCOMES=/genetics_work/perrotn/PURE/pPheWMR_consortia_V5/OUTCOMES_LIST_F11_BM_BM_MR.txt
awk 'BEGIN{FS=OFS="\t"}{print}' "$LIST_OUTCOMES" | tr -d '\r' > "${LIST_OUTCOMES}"temp
mv "${LIST_OUTCOMES}"temp "$LIST_OUTCOMES"
...
nohup bash /genetics/PAREG/perrotn/scripts/IVs_BM_MVMR_PURE_parallel.sh \
$SCRIPT \
$CIS_WINDOW \
...
$GENE \
$ROOT_DIR \
${ROOT_DIR}PURE/MVMR_BM_consortia/${GRS_FORMATING_FILE} \
$LIST_EXPOSURES \
$LIST_OUTCOMES \
...
grep -ci "error" /genetics/PAREG/perrotn/scripts/output_nohup/IVs_BM_MVMR_PURE_parallel.out
grep -i "fail" /genetics/PAREG/perrotn/scripts/output_nohup/IVs_BM_MVMR_PURE_parallel.out |  grep -cv "after larger attempt(s) failed" - 
grep -ci "cannot" /genetics/PAREG/perrotn/scripts/output_nohup/IVs_BM_MVMR_PURE_parallel.out
grep -ci "syntax error" /genetics/PAREG/perrotn/scripts/output_nohup/IVs_BM_MVMR_PURE_parallel.out

grep -in "error" /genetics/PAREG/perrotn/scripts/output_nohup/IVs_BM_MVMR_PURE_parallel.out | head
grep -i "fail" /genetics/PAREG/perrotn/scripts/output_nohup/IVs_BM_MVMR_PURE_parallel.out |  grep -nv "after larger attempt(s) failed" - 
grep -ni "cannot" /genetics/PAREG/perrotn/scripts/output_nohup/IVs_BM_MVMR_PURE_parallel.out | head
grep -in "syntax error" /genetics/PAREG/perrotn/scripts/output_nohup/IVs_BM_MVMR_PURE_parallel.out | head
